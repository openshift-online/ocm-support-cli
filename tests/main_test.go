...
// ocmSupportBinary is the path of the `ocm-support` binary that will be used in the tests.
var ocmSupportBinary string
var ocmCliBinary string
...
var _ = BeforeSuite(func() {
	...
	ocmSupportBinary = filepath.Join("..", name)
	_, err := os.Stat(ocmSupportBinary)
	Expect(err).ToNot(
		HaveOccurred(),
	...
	ocmCliBinary = filepath.Join("ocm", name)
	_, err = os.Stat(ocmCliBinary)
	Expect(err).ToNot(
		HaveOccurred(),
		"The '%s' binary doesn't exist, make sure to run 'make cmd' before running "+
			"these tests",
...
func (r *CommandRunner) Run(ctx context.Context, binary string) *CommandResult {
	var err error
